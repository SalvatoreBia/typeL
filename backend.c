...
static int fcount_lines(const char *filename)
{
    ...
    int count = 0;
    while (fgets(buf, WORD_MAX_LEN, file)) count++;
    fclose(file);
    return count;
}
...
void init_words_g(void)
{
    ...
    while (fgets(buf, WORD_MAX_LEN, file) && count < lines) {
        ...
        if (!words_g[count]) {
            ...
            for (int i = 0; i < count; i++) free(words_g[i]);
            free(words_g);
            fclose(file);
            exit(EXIT_FAILURE);
        }
...
static char **get_chunk(void)
{
    ...
    for (int i = 0; i < WORD_CHUNK; i++) {
        ...
        if (!chunk[i]) {
            ...
            for (int j = 0; j < i; j++) free(chunk[j]);
            free(chunk);
            exit(EXIT_FAILURE);
        }
        strcpy(chunk[i], tmp);
...
{
    if (!session) return;
    for (int i = 0; i < WORD_CHUNK; i++) free(session->list[i]);
    free(session->list);
...
session_list_t* create_session_list(void)
{
    ...
    for (int i = 0; i < MAX_SESSIONS; i++) list->sessions[i] = NULL;
    return list;
}

int add_session(session_list_t *list, session_t *session)
{
    if (!list || !session) return 0;

    pthread_mutex_lock(&list->lock);
    if (list->count == MAX_SESSIONS) { pthread_mutex_unlock(&list->lock); return 0; }
...
void remove_session(session_list_t *list, session_t *session)
{
    ...
    pthread_mutex_lock(&list->lock);
    if (list->count == 0) { pthread_mutex_unlock(&list->lock); return; }
...
{
    if (!list) { perror("***ERROR: session_list_t is null!"); exit(EXIT_FAILURE); }

    pthread_mutex_lock(&list->lock);
    if (list->count == MAX_SESSIONS) { pthread_mutex_unlock(&list->lock); return NULL; }
...
{
    if (!list || !session || !uuid_str) return;

    pthread_mutex_lock(&session->lock);

    if (session->players_count == 0) { pthread_mutex_unlock(&session->lock); return; }
    ...
    if (found && is_empty) {
        if (running) pthread_join(session->countdown_tid, NULL); 
...
{
    if (!session || !session->has_started || correct_words <= 0) return 0;
    ...
    int n = correct_words;
    if (n > WORD_CHUNK) n = WORD_CHUNK;

    size_t chars = 0;
    for (int i = 0; i < n; ++i) chars += strlen(session->list[i]);
    if (n > 1) chars += (size_t)(n - 1); 

    double wpm_f = (chars / 5.0) / (elapsed_s / 60.0);
    if (wpm_f < 0.0) wpm_f = 0.0;
